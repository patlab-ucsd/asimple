...
static uint32_t bmp280_get_t_fine(struct bmp280 *bmp280, uint32_t raw_temp)
{
	uint32_t t_fine;
	double var1, var2;
	var1 = (((double)raw_temp) / 16384.0 - ((double)bmp280->dig_T1) / 1024.0) *
		   ((double)bmp280->dig_T2);
	var2 =
		((((double)raw_temp) / 131072.0 - ((double)bmp280->dig_T1) / 8192.0) *
		 (((double)raw_temp) / 131072.0 - ((double)bmp280->dig_T1) / 8192.0)) *
		((double)bmp280->dig_T3);
...
)
{
	...
	var1 = (((double)bmp280->dig_P3) * var1 * var1 / 524288.0 +
			((double)bmp280->dig_P2) * var1) /
		   524288.0;
